...
export async function getOperlogList(params?: API.Monitor.OperlogListParams) {
  return request<API.Monitor.OperlogPageResult>('/admin/monitor/operlog/list', {
    method: 'GET',
    headers: {
...
export function getOperlog(operId: number) {
  return request<API.Monitor.OperlogInfoResult>(`/admin/monitor/operlog/${operId}`, {
    method: 'GET'
  });
}
...
export async function addOperlog(params: API.Monitor.Operlog) {
  return request<API.Result>('/admin/monitor/operlog', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json;charset=UTF-8',
    },
...
export async function removeOperlog(ids: string) {
  return request<API.Result>(`/admin/monitor/operlog/${ids}`, {
    method: 'DELETE'
  });
}

export async function cleanAllOperlog() {
  return request<API.Result>(`/admin/monitor/operlog/clean`, {
    method: 'DELETE'
  });
}
...
export function exportOperlog(params?: API.Monitor.OperlogListParams) {
  return downLoadXlsx(`/admin/monitor/operlog/export`, { params }, `operlog_${new Date().getTime()}.xlsx`);
}
