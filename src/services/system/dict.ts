...
export async function getDictTypeList(params?: API.DictTypeListParams) {
  return request(`/admin/system/dict/type/list`, {
    params: {
      ...params,
    },
    method: 'GET',
...
export function getDictType(dictId: string) {
  return request(`/admin/system/dict/type/${dictId}`, {
    method: 'GET',
  });
}
...
export async function getDictValueEnum(dictType: string, isDigital?: boolean): Promise<DictValueEnumObj> {
  const resp = await request<API.System.DictTypeResult>(`/admin/system/dict/data/type/${dictType}`, {
    method: 'GET',
  });
...
export async function addDictType(params: API.System.DictType) {
  return request<API.Result>('/admin/system/dict/type', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json;charset=UTF-8',
    },
...
export async function removeDictType(ids: string) {
  return request<API.Result>(`/admin/system/dict/type/${ids}`, {
    method: 'DELETE'
  });
}
...
export function exportDictType(params?: API.System.DictTypeListParams) {
  return downLoadXlsx(`/admin/system/dict/type/export`, { params }, `dict_type_${new Date().getTime()}.xlsx`);
}

// 获取字典选择框列表
export async function getDictTypeOptionSelect(params?: API.DictTypeListParams) {
  return request('/admin/system/dict/type/optionselect', {
    params: {
      ...params,
    },
    method: 'GET',
