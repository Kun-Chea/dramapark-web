...
const DictTag: React.FC<DictTagProps> = (props) => {
    ...
    function getDictListClassByValue(value: string | number | undefined): string {
        ...
        if (props.enums) {
            const item = props.enums[value];
            // 修复 item 可能为 undefined 的情况
            return item && item.listClass ? item.listClass : 'default';
        }
        if (props.options) {
            if (!Array.isArray(props.options)) {
                console.log('DictTag options is no array!')
    ...
    const getTagColor = () => {
        const listClass = getDictListClassByValue(props.value);
        return getDictColor(listClass ? listClass.toLowerCase() : 'default');
    };
