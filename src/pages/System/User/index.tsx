...
const UserTableList: React.FC = () => {
  ...
  const columns: ProColumns<API.System.User>[] = [
    ...
    },
    {
      title: '岗位',
      render: (_, record) => {
        return <span>{record.post.postName}</span>;
      },
    },
    {
    ...
    },
    {
      ...
      render: (_, record) => [
        ...
        <Button
          ...
          key="associate"
          icon=<UserOutlined />
          hidden={record.post.postCode != "group"}
          onClick={async () => {
    ...
    <PageContainer>
      ...
      <UpdateForm
        onSubmit={async (values) => {
          let success = false;
          let postIds = Array.isArray(values.postIds)
            ? values.postIds
            : values.postIds
              ? String(values.postIds)
                  .split(',')
                  .map((id) => id.trim())
              : [];
          // 只保留第一个postId，不再使用数组
          values = { ...values, postIds: postIds, postId: postIds[0] };
          if (values.userId) {
