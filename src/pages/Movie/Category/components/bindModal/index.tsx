...
import { listMovieCategoryByCategoryId } from '@/services/movie/category';
...
const BindModal: React.FC<BindModalProps> = ({ open, onClose, onOk, categoryId }) => {
  // 电影数据
  ...
  useAsyncEffect(async () => {
    if (open) {
      ...
      setLoading(true);
      try {
        const res = await listMovieCategoryByCategoryId(categoryId);
        const list = res?.data || [];
        setData(list);
        // 选中所有isBind为true的电影
